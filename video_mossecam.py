...
REC = False
resolution = (1920, 1080)  # or (1296, 972)
...
displayInfo = True
fileFormat = ["h264"]  # #format can be 'jpg', 'png', 'yuv', 'rgb', 'rgba', 'bgr', 'bgra'.
...
def shutter_button_press():
    global vidFileName, REC
    vidFileName = 'video_' + str(time.ctime()) + "." + str(fileFormat[0])
    REC = not REC

    disable_display()

    if REC == True:
        camera.start_recording(os.path.join('/home/pi/Desktop/MosseCam/Pictures', vidFileName), format=fileFormat[fileformatIndex])
    elif REC == False:
...
while True:
    ...
    if currentSettingIndexInt == 3 and displayInfo is False:
        disable_display()
